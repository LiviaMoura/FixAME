...
def parse_args(args):
    ...
    reqflags.add_argument(
        "-r2", metavar="R2", help="Reverse paired-end reads", required=True
    )
    ...
    curflags.add_argument(
        "-x",
        "--xtimes",
        help="Number of alignments during the curation [10]",
        default=10,
        metavar="INT",
        type=int,
    )
    curflags.add_argument(
        "-min",
        "--min_ctg_len",
        help="Minimun contig length [1000]",
        default=1000,
        metavar="INT",
        type=int,
    )
    ...
    curflags.add_argument(
        ...
        "--fasta_cov",
        metavar="INT",
        help="Local errors will be called on regions with coverage >= [5]",
        default=5,
        type=int,
    )
    curflags.add_argument(
        "--dp_cov",
        metavar="INT",
        help="Number of reads needed to extend the gaps/curate [1]",
        default=1,
        type=int,
    )
    curflags.add_argument(
        ...
        "--num_mismatch",
        metavar="INT",
        help="Number of mismatches allowed to filter out the initial reads [2]",
        default=2,
        type=int,
    )
    curflags.add_argument(
    ...
    curation_parser = subparsers.add_parser(
     ...
     """
        ...
        parents=[require_parser, curation_parent, system_parser],
        add_help=False,
    ...
    error_finder_parser = subparsers.add_parser(
        "error_finder",
        description=textwrap.dedent(
            """\
     ...
     """
        ...
        formatter_class=argparse.RawDescriptionHelpFormatter,
        parents=[require_parser, error_parent, system_parser],
        add_help=False,
        epilog=textwrap.dedent(
            """\
