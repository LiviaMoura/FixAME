...
(
    ...
    min_template_length,
    max_template_length,
) = partial_stats(bam_file, allowed_mismatches, threads)
...
def alternate_references(bam_file, num_mm, threads, max_template_length):
    ...
    for read in bam_parsed:
        ...
        if (
            reference != mate_reference and read.get_tag("NM") == 0
...
multi_reference_to_pair_dict = alternate_references(
    bam_file, allowed_mismatches, threads, max_template_length
)
