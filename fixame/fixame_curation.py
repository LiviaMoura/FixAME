    ...
    *Returns:
    ...
    if method == 0:
        ...
        for index,(sample_name,fasta_id) in df.iterrows():
            ...
            tmp_id.close()
            cmd = 'filterbyname.sh in={} out={} names={} include=t'.format(os.path.join(mydir,'fixame_results','bins_fixame.fasta'), os.path.join(mydir,'fixame_results',sample_name+'_fixame.fasta'), os.path.join(mydir,'tmp','tmp_fasta'))
            subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            cmd = 'filterbyname.sh in={} out={} names={} include=t'.format(os.path.join(mydir,'new_fastas','bins_renewed.fasta'), os.path.join(mydir,'new_fastas',sample_name+'_renewed.fasta'), os.path.join(mydir,'tmp','tmp_fasta'))
            subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def check_reads_N_edges(output_dir, contig_name, seq_mutable, seq_name, av_readlen,mean_gap, mean_gap_std, N_pos):
    ...
    for start, end, space in N_pos:
        ...
        if os.stat(left_right).st_size == 0:
            ...
            cmd = '''samtools view {}/check_read_sorted.bam {}:{}-{} | grep -F -f {}| awk -v FS="\\t" '$9 > 0 {{ sum += $9; n++ }} END {{print int(sum/n)}}' '''.format(os.path.join(output_dir,'tmp'), seq_name, (start_mgap+count), (end_mgap+1+count), left_right)
            #print(cmd)
