...
def final_output(output, df, features):
    ...
    num_events = len(df)
    num_fixed = len(df[df["status"] == "fixed"])
    num_chimeras = len(df[df["type_of_error"] == "possible chimera"])
    num_contigs = len(df["contig"].unique())
    logger.info(
        "\n\nFixAME could detect a total of {} errors in {} contig(s)\n * Fixed Errors: {} \n * Possible chimera errors: {}\n".format(
            num_events,
            num_contigs,
            num_fixed,
            num_chimeras,
